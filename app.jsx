...
function DocList({ docType, docs, onOpen, onNew, onDelete, data, onCreateFromSource, onCleanupBlanks }) {
  const [query, setQuery] = useState("");
  ...
  return (
    <div>
      ...
      {blankCount > 0 && (
        <div className="note" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, marginBottom: 14, marginTop: 0 }}>
          <span>入力内容が空の{DOC_META[docType].label}が {blankCount}件 あります。</span>
          <button className="btn btn-ghost btn-sm" onClick={onCleanupBlanks}>空の下書きを一括削除</button>
        </div>
      )}
      <div className="doc-table-wrap">
        {filtered.length === 0 ? (
...
function App() {
    ...
    if (DOC_ORDER.includes(activeTab) && activeDocId && (tab !== activeTab || docId !== activeDocId)) {
      ...
      if (current && isDocBlank(current)) {
        const discard = confirm(
          `この${DOC_META[activeTab].label}には何も入力されていません。\n` +
          `「OK」で破棄する / 「キャンセル」でこのまま一覧に残す`
        );
        if (discard) {
          setData(d => ({
            ...d,
            docs: { ...d.docs, [activeTab]: d.docs[activeTab].filter(x => x.id !== activeDocId) },
          }));
        }
      }
    }
  ...
  function cleanupBlankDocs(docType) {
    ...
    setData(d => ({ ...d, docs: { ...d.docs, [docType]: d.docs[docType].filter(x => !isDocBlank(x)) } }));
